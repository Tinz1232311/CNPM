...
public class GoogleLog extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (isEmailExists(email)) {
            // Nếu đã tồn tại, lấy vai trò của người dùng và chuyển hướng
            String role = UserRoleHelper.getUserRole(email);  // Gọi hàm từ lớp UserRoleHelper
            ...
            // Sau khi tạo tài khoản, lấy vai trò và chuyển hướng
            String role = UserRoleHelper.getUserRole(email);  // Gọi hàm từ lớp UserRoleHelper
            if ("admin".equals(role)) {
                response.sendRedirect("AdminServer");
            } else if ("employees".equals(role)) {
        ...
        try (Connection conn = DataConnect.getConnection()) {
            ...
            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                ...
                if (rs.next()) {
                    ...
                    System.out.println("Email does not exist: " + email);  // Email không tồn tại
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    // Chèn tài khoản mới vào CSDL và thêm vai trò 'customer' (role = 3)
